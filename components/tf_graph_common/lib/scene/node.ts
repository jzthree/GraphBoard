...
==============================================================================*/
module tfgraph.scene.node {
  import RenderNodeInfo = tfgraph.render.RenderNodeInfo;
  /**
...
export function getGroupSettingLabel(node: Node) {
  return tfgraph.getGroupSeriesNodeButtonString(
    getContainingSeries(node) !== null ? tfgraph.SeriesGroupingType.GROUP :
 ...
 */
export function traceInputs(renderGraphInfo: tfgraph.render.RenderGraphInfo) {
  // Reset all styling.
...
export function _getAllContainedOpNodes(
    nodeName: string, renderGraphInfo: tfgraph.render.RenderGraphInfo) {
  let opNodes = [];

  // Get current node.
  let node = renderGraphInfo.getNodeByName(nodeName) as tfgraph.GroupNode |
      tfgraph.OpNode;
  ...
  let childNodeNames = (node as tfgraph.GroupNode).metagraph.nodes();
...
export function traceAllInputsOfOpNode(
    renderGraphInfo: tfgraph.render.RenderGraphInfo, startNode: OpNode,
  ...
  _.each(inputs, function(nodeInstance) {
    ...
    if (resolvedNode instanceof MetanodeImpl) {
      let resolvedNodeName = tfgraph.getStrictName(resolvedNode.name);
  ...
  for (let index = 1; currentNode.name !== tfgraph.ROOT_NAME; index++) {
...
export function getVisibleParent(
    renderGraphInfo: tfgraph.render.RenderGraphInfo,
    currentNode: tfgraph.Node) {
  let found = false;
