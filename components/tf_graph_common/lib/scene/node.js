        ...
        (function (node_1) {
            ...
            function buildGroup(sceneGroup, nodeData, sceneElement) {
                ...
                nodeGroups
                    ...
                    addInteraction(label, d, sceneElement, d.node.type === tfgraph.NodeType.META);
            ...
            function getContextMenu(node, sceneElement) {
                var menu = [{
                        title: function (d) {
                            return tfgraph.getIncludeNodeButtonString(node.include);
                        },
            ...
            function getSeriesName(node) {
                ...
                if (node.type === tfgraph.NodeType.SERIES) {
                    return node.name;
                }
            ...
            function getContainingSeries(node) {
                ...
                else if (node.type === tfgraph.NodeType.SERIES) {
                    s = node;
                }
                else if (node.parentNode && node.parentNode.type === tfgraph.NodeType.SERIES) {
                    s = node.parentNode;
                }
                return s;
            }
            ...
            function getGroupSettingLabel(node) {
                return tfgraph.getGroupSeriesNodeButtonString(getContainingSeries(node) !== null ? tfgraph.SeriesGroupingType.GROUP :
                    tfgraph.SeriesGroupingType.UNGROUP);
            }
            ...
            function labelBuild(nodeGroup, renderNodeInfo, sceneElement) {
                ...
                var useFontScale = renderNodeInfo.node.type === tfgraph.NodeType.META &&
                ...
                switch (nodeType) {
                    case tfgraph.NodeType.META:
                        if (renderNodeInfo && !renderNodeInfo.expanded) {
                            // node expanded.
                            maxLength = tfgraph.layout.PARAMS.nodeSize.meta.maxLabelWidth;
                        }
                        break;
                    case tfgraph.NodeType.OP:
                        maxLength = tfgraph.layout.PARAMS.nodeSize.op.maxLabelWidth;
                        break;
                    case -1:
                        maxLength = tfgraph.layout.PARAMS.annotations.maxLabelWidth;
                        break;
                    default:
                        break;
                }
            ...
            function nodeClass(d) {
                switch (d.node.type) {
                    ...
                    case tfgraph.NodeType.ELLIPSIS:
            ...
            function getFillForNode(templateIndex, colorBy, renderInfo, isExpanded) {
                var colorParams = tfgraph.render.MetanodeColors;
                switch (colorBy) {
                    case ColorBy.STRUCTURE:
                        if (renderInfo.node.type === tfgraph.NodeType.META) {
                            var tid = renderInfo.node.templateId;
                            return tid === null ?
                                colorParams.UNKNOWN :
                                colorParams.STRUCTURE_PALETTE(templateIndex(tid), isExpanded);
                        }
                        else if (renderInfo.node.type === tfgraph.NodeType.SERIES) {
                        ...
                        else if (renderInfo.node.type === tfgraph.NodeType.BRIDGE) {
            ...
            function getStrokeForFill(fill) {
                ...
                return fill.substring(0, 3) === 'url' ?
                    tfgraph.render.MetanodeColors.GRADIENT_OUTLINE :
            ...
            function _getAllContainedOpNodes(nodeName, renderGraphInfo) {
                ...
                if (node instanceof tfgraph.OpNodeImpl) {
            ...
            function traceAllInputsOfOpNode(renderGraphInfo, startNode, allTracedNodes) {
                ...
                _.each(inputs, function (nodeInstance) {
                    ...
                    if (resolvedNode instanceof tfgraph.MetanodeImpl) {
                        var resolvedNodeName = tfgraph.getStrictName(resolvedNode.name);
                ...
                for (var index = 1; currentNode.name !== tfgraph.ROOT_NAME; index++) {
            ...
            function _markParentsOfNodes(visibleNodes) {
                _.forOwn(visibleNodes, function (nodeInstance) {
                    ...
                    var currentNode = nodeInstance;
                    while (currentNode.name !== tfgraph.ROOT_NAME) {
            ...
            function getVisibleParent(renderGraphInfo, currentNode) {
                ...
                while (!found) {
                    ...
                    if (currentParent === undefined) {
                        ...
                        if (renderNode &&
                            (renderNode.expanded || currentParent instanceof tfgraph.OpNodeImpl)) {
                            found = true;
                        }
                    }
                } // Close while loop.
