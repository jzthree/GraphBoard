...
(function (tfgraph) {
    ...
    })(GraphType = tfgraph.GraphType || (tfgraph.GraphType = {}));
    ;
    var NodeType;
    (function (NodeType) {
    ...
    })(NodeType = tfgraph.NodeType || (tfgraph.NodeType = {}));
    ;
    ...
    })(InclusionType = tfgraph.InclusionType || (tfgraph.InclusionType = {}));
    ;
    ...
    })(SeriesGroupingType = tfgraph.SeriesGroupingType || (tfgraph.SeriesGroupingType = {}));
    ;
    ...
    tfgraph.EllipsisNodeImpl = EllipsisNodeImpl;
    ;
    /**
    ...
    tfgraph.MetanodeImpl = MetanodeImpl;
    ;
    function createMetaedge(v, w) {
        return new MetaedgeImpl(v, w);
    }
    tfgraph.createMetaedge = createMetaedge;
    /**
    ...
    tfgraph.MetaedgeImpl = MetaedgeImpl;
    ...
    tfgraph.createSeriesNode = createSeriesNode;
    ...
    tfgraph.getSeriesNodeName = getSeriesNodeName;
    ...
    function build(rawNodes, params, tracker) {
        ...
        return tfgraph.util
            ...
            return tfgraph.util.runAsyncTask('Building the data structure', 70, function () {
    ...
    tfgraph.createGraph = createGraph;
    ;
    /**
    ...
    function getStrictName(name) {
        var parts = name.split(tfgraph.NAMESPACE_DELIM);
        return name + tfgraph.NAMESPACE_DELIM + '(' + parts[parts.length - 1] + ')';
    }
    tfgraph.getStrictName = getStrictName;
    /**
    ...
    function mapStrictHierarchy(nodeNames, embeddingNodeNames) {
        ...
        for (var i = 0; i < nodeNames.length - 1; ++i) {
            ...
            for (var j = i + 1; j < nodeNames.length; ++j) {
                var b = nodeNames[j];
                if (_.startsWith(b, a)) {
                    if (b.length > a.length && b.charAt(a.length) === tfgraph.NAMESPACE_DELIM) {
    ...
    tfgraph.hasSimilarDegreeSequence = hasSimilarDegreeSequence;
    ;
    /**
    ...
    function getHierarchicalPath(name, seriesNames) {
        var path = [];
        var i = name.indexOf(tfgraph.NAMESPACE_DELIM);
        // Push all parent portions of the path.
        while (i >= 0) {
            path.push(name.substring(0, i));
            i = name.indexOf(tfgraph.NAMESPACE_DELIM, i + 1);
        }
    ...
    tfgraph.getHierarchicalPath = getHierarchicalPath;
    ;
    /**
    ...
    function getIncludeNodeButtonString(include) {
        if (include === tfgraph.InclusionType.EXCLUDE) {
    ...
    tfgraph.getIncludeNodeButtonString = getIncludeNodeButtonString;
    ;
    /**
    ...
    function getGroupSeriesNodeButtonString(group) {
        if (group === tfgraph.SeriesGroupingType.GROUP) {
    ...
    tfgraph.getGroupSeriesNodeButtonString = getGroupSeriesNodeButtonString;
    ;
    /**
    ...
    function toggleNodeSeriesGroup(map, name) {
        if (!(name in map) || map[name] === tfgraph.SeriesGroupingType.GROUP) {
            map[name] = tfgraph.SeriesGroupingType.UNGROUP;
        }
        else {
            map[name] = tfgraph.SeriesGroupingType.GROUP;
        }
    }
    tfgraph.toggleNodeSeriesGroup = toggleNodeSeriesGroup;
    ;
})(tfgraph || (tfgraph = {})); // close module tfgraph
