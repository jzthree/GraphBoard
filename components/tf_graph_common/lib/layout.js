...
(function (tfgraph) {
    ...
    (function (layout) {
        ...
        function layoutScene(renderNodeInfo) {
            ...
            else if (renderNodeInfo.node.type === tfgraph.NodeType.SERIES) {
        ...
        function updateTotalWidthOfNode(renderInfo) {
            ...
            var labelLength = renderInfo.node.name.length -
                renderInfo.node.name.lastIndexOf(tfgraph.NAMESPACE_DELIM) - 1;
        ...
        function layoutChildren(renderNodeInfo) {
            ...
            _.each(children, function (childNodeInfo) {
                ...
                switch (childNodeInfo.node.type) {
                    case tfgraph.NodeType.OP:
                        _.extend(childNodeInfo, layout.PARAMS.nodeSize.op);
                        break;
                    case tfgraph.NodeType.BRIDGE:
        ...
        function dagreLayout(graph, params) {
            ...
            _.each(graph.nodes(), function (nodeName) {
                ...
                if (nodeInfo.node.type === tfgraph.NodeType.BRIDGE) {
        ...
        function sizeAnnotation(a) {
            switch (a.annotationType) {
                case tfgraph.render.AnnotationType.CONSTANT:
                    _.extend(a, layout.PARAMS.constant.size);
                    break;
                case tfgraph.render.AnnotationType.SHORTCUT:
                    if (a.node.type === tfgraph.NodeType.OP) {
                        _.extend(a, layout.PARAMS.shortcutSize.op);
                    }
                    else if (a.node.type === tfgraph.NodeType.META) {
                        _.extend(a, layout.PARAMS.shortcutSize.meta);
                    }
                    else if (a.node.type === tfgraph.NodeType.SERIES) {
                    ...
                    break;
                case tfgraph.render.AnnotationType.SUMMARY:
                    _.extend(a, layout.PARAMS.constant.size);
                    break;
            }
        }
    ...
    })(layout = tfgraph.layout || (tfgraph.layout = {}));
})(tfgraph || (tfgraph = {})); // close module
