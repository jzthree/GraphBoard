...
(function (tfgraph) {
    ...
    (function (render) {
        ...
        var RenderGraphInfo = (function () {
            ...
            RenderGraphInfo.prototype.computeScales = function () {
                ...
                this.edgeWidthScale = this.hierarchy.hasShapeInfo ?
                    ...
                    d3.scale.linear()
                        ...
                        .range([tfgraph.scene.edge.MIN_EDGE_WIDTH, tfgraph.scene.edge.MAX_EDGE_WIDTH]);
            };
            /**
            ...
            RenderGraphInfo.prototype.getNearestVisibleAncestor = function (name) {
                var path = tfgraph.getHierarchicalPath(name);
            ...
            RenderGraphInfo.prototype.buildSubhierarchy = function (nodeName) {
                ...
                if (renderNodeInfo.node.type !== tfgraph.NodeType.META &&
                    renderNodeInfo.node.type !== tfgraph.NodeType.SERIES) {
                    return;
                }
                ...
                if (PARAMS.enableExtraction &&
                    renderGroupNodeInfo.node.type === tfgraph.NodeType.META) {
                ...
                _.each([true, false], function (inbound) {
                    ...
                    _.each(coreGraph.nodes(), function (childName) {
                        ...
                        if (childNodeInfo.node.type === tfgraph.NodeType.BRIDGE) {
                            return;
                        }
        ...
        var AnnotationList = (function () {
            ...
            AnnotationList.prototype.push = function (annotation) {
                ...
                var ellipsisNode = new tfgraph.EllipsisNodeImpl(1);
            ...
            _.each(graph.nodes(), function (nodeName) {
                ...
                if (depth > 0) {
                    switch (child.node.type) {
                        case tfgraph.NodeType.META:
                        case tfgraph.NodeType.SERIES:
        ...
        var RenderGroupNodeInfo = (function (_super) {
            ...
            function RenderGroupNodeInfo(groupNode) {
                ...
                _this.coreGraph =
                    tfgraph.createGraph(gl.name, tfgraph.GraphType.CORE, { compound: true });
        ...
        function createShortcut(graph, v, w) {
            ...
            if ((src.node.include === tfgraph.InclusionType.INCLUDE ||
                sink.node.include === tfgraph.InclusionType.INCLUDE) &&
                src.node.include !== tfgraph.InclusionType.EXCLUDE &&
                sink.node.include !== tfgraph.InclusionType.EXCLUDE) {
                return;
            }
            // Add each annotation.
        ...
        function makeOutExtract(renderNode, n, forceDetach) {
            ...
            if (graph.neighbors(n).length === 0) {
                child.node.include = tfgraph.InclusionType.EXCLUDE;
        ...
        function hasTypeIn(node, types) {
            if (node.type === tfgraph.NodeType.OP) {
            ...
            else if (node.type === tfgraph.NodeType.META) {
        ...
        function extractSpecifiedNodes(renderNode) {
            ...
            _.each(graph.nodes(), function (n) {
                ...
                if (renderInfo.node.include === tfgraph.InclusionType.EXCLUDE) {
        ...
        function extractPredefinedSink(renderNode) {
            ...
            _.each(graph.nodes(), function (n) {
                var renderInfo = graph.node(n);
                if (renderInfo.node.include !== tfgraph.InclusionType.UNSPECIFIED) {
                    return;
                }
        ...
        function extractHighInOrOutDegree(renderNode) {
            ...
            _.each(graph.nodes(), function (currentNode) {
                if (graph.node(currentNode).node.include !== tfgraph.InclusionType.UNSPECIFIED) {
        ...
        function extractHighDegrees(renderNode) {
            ...
            _.each(graph.nodes(), function (n) {
                ...
                if (child.node.include !== tfgraph.InclusionType.UNSPECIFIED) {
                    return;
                }
                if (degree === 0) {
    ...
    })(render = tfgraph.render || (tfgraph.render = {}));
})(tfgraph || (tfgraph = {})); // close module tfgraph.render
