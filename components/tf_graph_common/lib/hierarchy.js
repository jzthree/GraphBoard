...
(function (tfgraph) {
    ...
    (function (hierarchy_1) {
        ...
        var HierarchyImpl = (function () {
            function HierarchyImpl() {
                 ...
                 */
                this.index = {};
                this.index[tfgraph.ROOT_NAME] = this.root;
                this.orderings = {};
            }
            ...
            HierarchyImpl.prototype.getBridgegraph = function (nodeName) {
                ...
                var bridgegraph = groupNode.bridgegraph =
                    tfgraph.createGraph('BRIDGEGRAPH', tfgraph.GraphType.BRIDGE);
                ...
                _.each([parentMetagraph, parentBridgegraph], function (parentGraph) {
                    _(parentGraph.edges())
                        ...
                        _.each(parentMetaedge.baseEdgeList, function (baseEdge) {
                            ...
                            if (!bridgeMetaedge) {
                                bridgeMetaedge = tfgraph.createMetaedge(bridgeEdgeObj.v, bridgeEdgeObj.w);
                    ...
                    _.each(node.inEmbeddings, function (embeddedNode) {
                        _.each(node.inputs, function (input) {
                            if (input.name === embeddedNode.name) {
                                ...
                                var metaedge = new tfgraph.MetaedgeImpl(embeddedNode.name, nodeName);
                    ...
                    _.each(node.outEmbeddings, function (embeddedNode) {
                        _.each(embeddedNode.inputs, function (input) {
                            if (input.name === nodeName) {
                                ...
                                var metaedge = new tfgraph.MetaedgeImpl(nodeName, embeddedNode.name);
                                metaedge.addBaseEdge({
                                    isControlDependency: input.isControlDependency,
                                    outputTensorIndex: input.outputTensorIndex,
                                    isReferenceEdge: false,
        ...
        function build(graph, params, tracker) {
            ...
            return tfgraph.util
                ...
                .then(function () {
                return tfgraph.util.runAsyncTask('Detect series', 20, function () {
                ...
                .then(function () {
                return tfgraph.util.runAsyncTask('Adding edges', 30, function () {
                    addEdges(h, graph, seriesNames);
                }, tracker);
            })
                .then(function () {
                return tfgraph.util.runAsyncTask('Finding similar subgraphs', 30, function () {
                    h.templates = tfgraph.template.detect(h, params.verifyTemplate);
                }, tracker);
            })
        ...
        function addNodes(h, graph) {
            _.each(graph.nodes, function (node, nodeName) {
                var path = tfgraph.getHierarchicalPath(node.name);
                var parent = h.root;
                ...
                for (var i = 0; i < path.length; i++) {
                    ...
                    if (!child) {
                        child = tfgraph.createMetanode(name_1);
        ...
        function addEdges(h, graph, seriesNames) {
            ...
            _.each(graph.edges, function (baseEdge) {
                ...
                if (!metaedge) {
                    metaedge = tfgraph.createMetaedge(sourceAncestorName, destAncestorName);
        ...
        function detectSeries(clusters, metagraph) {
            ...
            _.each(clusters, function (members, clusterId) {
                ...
                _.each(members, function (name) {
                    ...
                    var seriesName = tfgraph.getSeriesNodeName(prefix, suffix, parent);
                    candidatesDict[seriesName] = candidatesDict[seriesName] || [];
                    var seriesNode = tfgraph.createSeriesNode(prefix, suffix, parent, +id, name);
        ...
        function addSeriesToDict(seriesNodes, seriesDict, clusterId, metagraph) {
            if (seriesNodes.length > 1) {
                var curSeriesName = tfgraph.getSeriesNodeName(seriesNodes[0].prefix, seriesNodes[0].suffix, seriesNodes[0].parent, seriesNodes[0].clusterId, seriesNodes[seriesNodes.length - 1].clusterId);
                var curSeriesNode_1 = tfgraph.createSeriesNode(seriesNodes[0].prefix, seriesNodes[0].suffix, seriesNodes[0].parent, clusterId, curSeriesName);
    ...
    })(hierarchy = tfgraph.hierarchy || (tfgraph.hierarchy = {}));
})(tfgraph || (tfgraph = {})); // close module tfgraph.hierarchy
